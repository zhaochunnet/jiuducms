...
<meta http-equiv='Refresh' content='<{$waitSecond}>;URL=javascript:<{$jumpUrl}>'>
<style>
...
<div class="message">
	...
	<present name="closeWin" >
		页面将在 <span class="wait" id="time"><{$waitSecond}></span> 秒后自动关闭，如果不想等待请点击 <a href="javascript:<{$jumpUrl}>">这里</a> 关闭
	<else/>
		页面将在 <span class="wait" id="time"><{$waitSecond}></span> 秒后自动跳转，如果不想等待请点击 <a href="javascript:<{$jumpUrl}>">这里</a> 跳转
	</present>
	</div>
</div>
<script language="javascript">
	var oTime=document.getElementById('time');
	var oSecs=<{$waitSecond}>;
	function time(){
        oSecs --;
        if (oSecs < 0){
			clearInterval();
        ...
        if (oSecs == 0){
			...
			<present name="closeWin" >
				window.close();
			<else/>
			<eq name="jumpUrl" value="history.back(-1);">
				history.back(-1);
			<else/>
