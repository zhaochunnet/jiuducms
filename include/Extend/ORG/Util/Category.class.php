 ...
 * @version   $Id: Category.class.php 2012-11-7 14:59:02Z struggle $
 +------------------------------------------------------------------------------
 */
class Category {
     ...
     * @param string        $orderby    排序
      +----------------------------------------------------------
     */
    private function _findAllCat($condition,$orderby = NULL,$field=true) {
        $this->rawList = empty($orderby) ? $this->model->where($condition)->field($field)->select() : $this->model->where($condition)->order($orderby)->field($field)->select();
    }
     ...
     * @return  array           返回结构信息
      +----------------------------------------------------------
     */
    public function getList($field = true,$condition = NULL,$cid = 0, $orderby = NULL) {
        unset($this->rawList, $this->formatList);
        $this->_findAllCat($condition,$orderby,$field);
