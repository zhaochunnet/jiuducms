...
class  ThinkTemplate {
    ...
    private function parseIncludeItem($tmplPublicName,$vars=array()){
        // 分析模板文件名并读取内容
        if(C('DIY_TEMPLATE_NAME') === true){
        	$parseStr = $this->parseDiyTemplateName($tmplPublicName);
        }elseif((APP_NAME == 'Admin' && MODULE_NAME != 'Makehtml')){
        	$parseStr = $this->parseTemplateName($tmplPublicName);
        }else{
        	$path = $vars['path'];
        	$parseStr = $this->parseDiyTemplateName($tmplPublicName,$path);
        	unset($vars['path']);
        }
        // 替换变量
        foreach ($vars as $key=>$val) {
    ...
    /**
     * 自定义分析加载的模板文件并读取内容 支持多个模板文件读取
     * @access private
     * @param string $tmplPublicName  模板文件名
     * @return string
     */
    private function parseDiyTemplateName($templateName,$path=''){
    	if(substr($templateName,0,1)=='$'){
    		//支持加载变量文件名
    		$templateName = $this->get(substr($templateName,1));
    	}
    	$array  =   explode(',',$templateName);
    	$parseStr   =   '';
    	$pathname = $path ? SITE_PATH.'/'.C('TEMPLETS_PATH').'/'.$path.'/' : SITE_PATH.'/'.C('TEMPLETS_PATH').'/'.C('TEMPLETS_STYLE').'/'.C('TEMPLETS_CHECK_TYPE').'/';
    	foreach ($array as $templateName){
